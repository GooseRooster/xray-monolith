...
extern u32 g_r;
extern int ps_r3_ssfx_shadows;  // OWA: SSFX shadows runtime toggle
...
void CRender::Render()
{
	...
	// OWA: Skip sun shadow cascades in static sun mode OR static lighting mode (R1-style lightmaps)
	if (o.sunstatic || o.staticlighting) bSUN = FALSE;
	...
	// OWA: Skip wet surfaces in static lighting mode (R1 aesthetic - simple gloss only)
	if (ps_r2_ls_flags.test(R3FLAG_DYN_WET_SURF) && !o.staticlighting)
	{
...

	{
		...
		// OWA: o.ssfx_sss now includes r3_ssfx_shadows check (compile-time)
